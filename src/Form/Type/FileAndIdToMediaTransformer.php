...
class FileAndIdToMediaTransformer implements DataTransformerInterface
{
    /** @var null|string */
    protected $mediaName;
    /**
    ...
    public function reverseTransform($value)
    {
        ...
        if (!empty($value['ent']) && $value['ent']->getId() === (int) $value['id']) {
            $this->currentValueContainer->setCurrentValue($value['ent']);

            return $value['ent'];
        }

        if (!($entity = $this->objectManager->getRepository(Media::class)->find($value['id']))) {
